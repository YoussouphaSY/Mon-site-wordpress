 ...
 * @return array
 */
function soccerclubacademy_body_classes( $classes ) {
...
add_filter( 'body_class', 'soccerclubacademy_body_classes' );
...
if (!function_exists('soccerclubacademy_str_replace_assoc')) {

    /**
     * soccerclubacademy_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function soccerclubacademy_str_replace_assoc(array $replace, $subject) {
 ...
 */
function soccerclubacademy_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html_e('0','soccer-club-academy'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'soccerclubacademy_add_to_cart_fragment' );
...
if ( ! function_exists( 'soccerclubacademy_breadcrumb_title' ) ) {
	function soccerclubacademy_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'soccer-club-academy' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'soccer-club-academy' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'soccer-club-academy' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'soccer-club-academy' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'soccer-club-academy' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'soccer-club-academy' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'soccer-club-academy' ), (get_the_author( '', false ) ));		
...
function soccerclubacademy_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','soccer-club-academy'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','soccer-club-academy'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','soccer-club-academy'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','soccer-club-academy').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','soccer-club-academy').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','soccer-club-academy').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','soccer-club-academy').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','soccer-club-academy'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','soccer-club-academy') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_soccerclubacademy_dismissed_notice_handler', 'soccerclubacademy_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function soccerclubacademy_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'soccerclubacademy_admin_install_plugin' );

function soccerclubacademy_admin_install_plugin() {
    /**
     * Install Plugin.
     */
