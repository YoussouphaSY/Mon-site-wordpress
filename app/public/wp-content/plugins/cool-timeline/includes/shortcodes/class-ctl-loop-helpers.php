...
if ( ! class_exists( 'CTL_Loop_Helpers' ) ) {
	...
	class CTL_Loop_Helpers {
		...
		public function ctl_render( $index, $post ) {
			...
			$output .= '<!-- Timeline Content --><div  id="ctl-story-' . esc_attr( $post_id ) . '" class="' . esc_attr( implode( ' ', $classes ) ) . '" data-aos="' . esc_attr( $animation ) . '"   data-story-index="' . esc_attr( $index ) . '">';

			if ( 'horizontal' === $layout ) {
				$output .= '<!-- Line Filling -->
							<div class="ctl-story-line"></div>';
			}
			if ( 'compact' !== $layout ) {
